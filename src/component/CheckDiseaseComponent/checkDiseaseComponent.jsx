...
const DiseasePredictionComponent = () => {
  ...
  const [precautions, setPrecautions] = useState(''); // State for precautions
  ...
  const handlePredict = async () => {
    if (symptoms.length === 0) {
      ...
      try {
        ...
        // Assuming response data contains a list of predictions with confidence scores and precautions
        const { disease, probability, precautions } = response.data[0];

        // Update state with the prediction results and precautions
        setPredictedDisease(disease);
        setConfidenceScore(probability * 100); // Convert to percentage
        setPrecautions(precautions); // Set precautions
            ...
            <div className="text-center mb-4">
              ...
              <div className="mt-4">
                ...
                <div className="bg-gray-100 p-4 rounded-lg mt-2 max-h-40 overflow-y-auto">
                  <p>{precautions || 'No precautions available'}</p>
                </div>
              </div>
            </div>
